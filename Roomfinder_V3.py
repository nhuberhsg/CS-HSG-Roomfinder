...
st.write(f":arrow_right: You're looking for a room at {formatted_start_time} for {duration} hours")
...
#### Display DF
	...
	st.table(green_rooms.rename(columns={'shortName': 'Room'})[['Status', 'Room', 'Note']].style.applymap(status_to_color, subset=['Status']).set_table_styles([{'selector': f'.col{i}', 'props': [('width', f'{column_widths.get(col, 150)}px')] } for i, col in enumerate(['Status', 'Room', 'Note'])]))
...
# Implement Error Message to draw the attention of the user to the opening hours of HSG
	...
	st.markdown(
		f'<div style="color: white; font-size: 18px; font-weight: bold;">{error_message}</div>',
		unsafe_allow_html=True
	)
	st.markdown(
		f'<div style="color: white; font-size: 18px; font-weight: bold;">{note}</div>',
		unsafe_allow_html=True
	)